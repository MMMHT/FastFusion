...
train_loader = DataLoader(DLRF.RFLoader(cfg.datapath, 'train', (352, 1216)), batch_size=cfg.batchsize,
...
model.load_state_dict(torch.load('./workspace/RFslim2_Sceneflow_selected/ckpt/2015_fbnn.pth', map_location='cuda:0'))
...
def get_loss(pred, img, groundtruth, input,  epoches, a = 0.5 ):
...
def train():
    ...
    for epoch in range(cfg.epoch):
        ...
        for idx, data in enumerate(tqdm(train_loader)):
            ...
            inputs['left_pre'] = data['left_pre']
            # inputs['right_pre'] = data['right_pre']
            inputs['left_disp'] = data['left_disp'][:,:1,:,:]
            inputs['right_disp'] = data['right_disp'][:,:1,:,:]
            inputs['left_leaDisp'] = data['left_leaDisp']

            optimizer.zero_grad()

            inp = torch.cat((inputs['left_pre'], inputs['left_img'],  inputs['left_slidar']), 1)
            pred = model(inp)
            loss = get_loss(pred, inputs['left_img'], inputs['left_leaDisp'], inputs['left_pre'], epoch)
            loss.backward()
