...
class Metrics(object):
    """ Benchmark """
    def __init__(self, field_names):
        """ Metrics to be evaluated are specified in `field_names`.  pred_bad:gt(groundTruth, 1):cmul(mask)
...
def rmse(pred, target):
    ...
    # mask_pred = pred>0
    # mask_tar = target>0
    # mask = mask_tar*mask_pred
    # return math.sqrt(np.power((f*b/pred[mask] - f*b/target[mask]), 2).mean())
